...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $_SESSION['checkout_data'] = [
        ...
        'member' => $_POST['member'],
        'type' => $_POST['type'],
        'amount' => ($_POST['type'] === 'VIP Cheki' ? '70.000' : '35.000')
    ];
...
<section class="pt-40 pb-20 bg-background min-h-screen">
    <div class="container mx-auto px-6 max-w-4xl">
        ...
        <div class="bg-white p-8 md:p-12 rounded-4xl shadow-2xl border border-neutral/5">
            <form action="checkout.php" method="POST" enctype="multipart/form-data" class="space-y-8">
                <!-- Detail Pemesan -->
                <div>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <select id="member" name="member" class="w-full bg-background border border-neutral/10 rounded-xl px-4 py-3 focus:outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all text-neutral appearance-none" required>
                                <option value="" disabled <?php echo !isset($_GET['member']) ? 'selected' : ''; ?>>Pilih Member...</option>
                                <?php foreach($members as $m): ?>
                                    <option value="<?php echo strtolower($m['name']); ?>" <?php echo (isset($_GET['member']) && $_GET['member'] == strtolower($m['name'])) ? 'selected' : ''; ?>>
                                        <?php echo $m['name']; ?>
                                    </option>
                                <?php <?php endforeach; ?>
                            </select>
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="w-full border-2 border-dashed border-neutral/20 rounded-xl p-8 text-center hover:border-primary/50 transition-colors cursor-pointer bg-background" onclick="document.getElementById('proof').click()">
