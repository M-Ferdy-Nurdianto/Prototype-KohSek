...
<div class="group">
    <div class="relative mb-8 rounded-4xl overflow-hidden shadow-xl aspect-[3/4] transition-transform duration-500 hover:-translate-y-2">
        <img src="{{ asset($member['image']) }}" alt="{{ $member['name'] }}" class="w-full h-full object-cover object-center group-hover:scale-110 transition-transform duration-700">
    ...
    <div class="space-y-2">
        <div class="flex justify-between items-end">
            <h3 class="text-3xl font-extrabold text-neutral">{{ $member['full_name'] }}</h3>
            <span class="text-primary font-bold text-sm uppercase tracking-widest pb-1">{{ $member['role'] }}</span>
        </div>
        <p class="text-neutral/60 leading-relaxed italic">{{ $member['description'] }}</p>
    </div>
</div>